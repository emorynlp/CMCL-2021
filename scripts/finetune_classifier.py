...
from gluonnlp.model import BERTProbingClassifierMHP
...
parser.add_argument(
    '--surfix', type=str, default='.bert.csv', help='data file surfix')

parser.add_argument(
    '--dropout', type=float, default=0.0, help='droupout rate')

parser.add_argument(
    '--use_pooler', default=True)

parser.add_argument(
    '--output_emb', default=False)

parser.add_argument(
    '--num_hidden_prob', type=int, default=0, help='number of hidden probers')

parser.add_argument(
    '--dims_hidden_prob', type=int, default=0, help='dims of hidden probers')
...
get_model_params['use_pooler'] = args.use_pooler
...
if do_regression:
    ...
    loss_2 = gluon.loss.L1Loss()

# parse dim_mlps
if args.dim_mlps != None and args.dim_mlps != '_' and args.dim_mlps != '0':
    dim_mlps = [int(dim) for dim in args.dim_mlps.split('_')]
else:
    dim_mlps = []
...
if use_roberta:
    ...
    if modelname == 'baseline':
        model = BERTClassifier(bert, dropout=0.0, num_classes=num_classes)
    elif modelname == 'MHP':
        model = BERTProbingClassifierMHP(bert, dropout=args.dropout, num_classes=num_classes,dim_hidden_mlps=dim_mlps,
                        num_hidden_prob=args.num_hidden_prob, dims_hidden_prob=args.dims_hidden_prob)
...
if not model_parameters:
    model.init_params(initializer=initializer, ctx=ctx)
...
loss_1.hybridize(static_alloc=True)
loss_2.hybridize(static_alloc=True)
lmbd = 0.5
...
def write_output_emb(filename, sent_emb):
    emb_path = os.path.join(args.output_dir, filename)
    logging.info('write sentence embedding to %s', emb_path)
    with io.open(emb_path, 'w', encoding='utf-8') as f:
        for embs in sent_emb:
            for emb in embs:
                emb = emb.asnumpy().tolist()
                f.write(u'%s\n'%('\t'.join(['{}'.format(x) for x in emb])))


def test(loader_test, segment, epoch_id='none'):
    ...
    for _, seqs in enumerate(loader_test):
        ...
        if use_roberta:
            ...
            out, emb, h_128_emb, h_64_emb, h_32_emb = model(input_ids, segment_ids.as_in_context(ctx), valid_length)
            sent_emb.append(emb) # hidden layer output
            h_128_sent_emb.append(h_128_emb)
            h_64_sent_emb.append(h_64_emb)
            h_32_sent_emb.append(h_32_emb)
    ...
    filename = args.task_name + segment.replace('test', '') + '_'  + str(epoch_id) + '.tsv'
    test_path = os.path.join(args.output_dir, filename)
    logging.info('write test result to %s', test_path)
    with io.open(test_path, 'w', encoding='utf-8') as f:
        for i, pred in enumerate(results):
            f.write(u'%d\t%s\n'%(i, str(pred)))

    if args.output_emb:
        filename = args.task_name + segment.replace('test', '') + '_'  + str(epoch_id) + '.norm.emb.tsv'
        write_output_emb(filename, sent_emb)
        filename = args.task_name + segment.replace('test', '') + '_'  + str(epoch_id) + '.128.emb.tsv'
        write_output_emb(filename, h_128_sent_emb)
        filename = args.task_name + segment.replace('test', '') + '_'  + str(epoch_id) + '.64.emb.tsv'
        write_output_emb(filename, h_64_sent_emb)
        filename = args.task_name + segment.replace('test', '') + '_'  + str(epoch_id) + '.32.emb.tsv'
        write_output_emb(filename, h_32_sent_emb)
...
def log_train(batch_id, batch_num, metric, step_loss, log_interval, epoch_id, learning_rate):
    ...
    logging.info(train_str, epoch_id, batch_id + 1, batch_num,
...
def train(metric):
    ...
    for epoch_id in range(args.epochs):
            ...
            for batch_id, seqs in enumerate(train_data):
                ...
                with mx.autograd.record():
                    ...
                    if use_roberta:
                        ...
                        out, _, _, _, _ = model(input_ids, segment_ids.as_in_context(ctx), valid_length)
        ...
        #    test(test_data, segment, epoch_id)
