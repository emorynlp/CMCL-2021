...
__all__ = ['BERTModel', 'RoBERTaModel', 'BERTEncoder', 'BERTClassifier','BERTProbingClassifierMHP',
...
import numpy as np
import mxnet as mx
from mxnet.gluon import HybridBlock, Block, nn
...
class BERTEncoder(HybridBlock, Seq2SeqEncoder):
    ...
    def hybrid_forward(self, F, inputs, states=None, valid_length=None, position_weight=None):
        ...
        for cell in self.transformer_cells:
            ...
            raw_transformer_outputs = outputs
            inputs = outputs
        ...
        if self._output_all_encodings:
            return all_encodings_outputs, additional_outputs, raw_transformer_outputs

        return outputs, additional_outputs, raw_transformer_outputs
...
class BERTModel(HybridBlock):
    ...
    def hybrid_forward(self, F, inputs, token_types, valid_length=None, masked_positions=None):
        ...
        """
        outputs = []
        seq_out, attention_out, raw_transformer_outputs = self._encode_sequence(inputs, token_types, valid_length)
        ...
        cls_embs = []
        for seq in seq_out:
           cls_embs.append(self._apply_pooling(seq))

        outputs.append(cls_embs)
    ...
    def _encode_sequence(self, inputs, token_types, valid_length=None):
        ...
        # encoding
        outputs, additional_outputs, raw_transformer_outputs = self.encoder(embedding, valid_length=valid_length)
        return outputs, additional_outputs, raw_transformer_outputs
...
class BERTClassifier(HybridBlock):
    ...
    def hybrid_forward(self, F, inputs, token_types, valid_length=None):
        ...
        outputs : NDArray
            Shape (batch_size, num_classes)
        """
        _, _, pooler_out, _ = self.bert(inputs, token_types, valid_length)
        return self.classifier(pooler_out), pooler_out

    def init_params(self, initializer, ctx):

        self.classifier.initialize(init=initializer, ctx=ctx)
...
def get_bert_model(model_name=None, dataset_name=None, vocab=None, pretrained=True, ctx=mx.cpu(),
                   use_pooler=True, use_decoder=True, use_classifier=True, output_attention=True,
...
# adjustable number of mlps for each prober
class BERTProbingClassifierMHP(HybridBlock):
    def __init__(self, bert, num_classes=2, dropout=0.0,
                 prefix=None, params=None, dim_hidden_mlps=[], num_hidden_prob=4, dims_hidden_prob=32, ctx=None):
        super(BERTProbingClassifierMHP, self).__init__(prefix=prefix, params=params)

        self.bert = bert

        self.hidden_prob_mlps = nn.HybridSequential(prefix=prefix) # probing hidden layer

        with self.name_scope():
            for i in range(num_hidden_prob):
                l = nn.HybridSequential(prefix=prefix)

                if dropout:
                    l.add(nn.Dropout(rate=dropout))

                for dim_mlp in dim_hidden_mlps:
                    l.add(nn.Dense(units=dim_mlp))

                l.add(nn.Dense(units=dims_hidden_prob))
                self.hidden_prob_mlps.add(l)

            self.output_layer = nn.HybridSequential(prefix=prefix)

            if dropout:
                self.output_layer.add(nn.Dropout(rate=dropout))
            self.output_layer.add(nn.Dense(units=num_classes))

            self.layer_norm_prob = nn.LayerNorm()

    def __call__(self, inputs, token_types, valid_length=None):
        return super(BERTProbingClassifierMHP, self).__call__(inputs, token_types, valid_length)
    ...
    def hybrid_forward(self, F, inputs, token_types, valid_length=None):
        ...
        for i, hd_layer in enumerate(self.hidden_prob_mlps):
            if i == 0:
                h_prob_out = hd_layer(pooler_out)

                ## 3 layer
                h_128_h_out = hd_layer[0](pooler_out)
                h_64_h_out = hd_layer[1](h_128_h_out)
                h_32_h_out = hd_layer[2](h_64_h_out )

            else:
                h_prob_out = mx.symbol.Concat(h_prob_out, hd_layer(pooler_out), dim=1)

                ## 3 layer
                tmp_h_128_h_out = hd_layer[0](pooler_out)
                tmp_h_64_h_out  = hd_layer[1](tmp_h_128_h_out)
                tmp_h_32_h_out  = hd_layer[2](tmp_h_64_h_out)
                h_128_h_out = mx.symbol.Concat(h_128_h_out, tmp_h_128_h_out, dim=1)
                h_64_h_out = mx.symbol.Concat(h_64_h_out, tmp_h_64_h_out, dim=1)
                h_32_h_out = mx.symbol.Concat(h_32_h_out, tmp_h_32_h_out, dim=1)


        h_prob_out = self.layer_norm_prob(h_prob_out)
        output = self.output_layer(h_prob_out)
        #return output, h_prob_out
        return output, pooler_out, h_128_h_out, h_64_h_out, h_32_h_out

    def init_params(self, initializer, ctx):

        for layer in self.hidden_prob_mlps:
            layer.initialize(init=initializer, ctx=ctx)

        self.output_layer.initialize(init=initializer, ctx=ctx)

        self.layer_norm_prob.initialize(init=initializer, ctx=ctx)
