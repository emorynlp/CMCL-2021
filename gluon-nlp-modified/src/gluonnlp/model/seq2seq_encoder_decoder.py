...
from .attention_cell import (AttentionCell, DotProductAttentionCell,
                            ...
                            MyMultiHeadAttentionCell2,
                            MyMultiHeadAttentionCell)
...
def _get_attention_cell(attention_cell, units=None,
    ...
    if isinstance(attention_cell, str):
        ...
        elif attention_cell == 'my_multi_head':
            base_cell = DotProductAttentionCell(scaled=scaled, dropout=dropout)
            return MyMultiHeadAttentionCell(base_cell=base_cell, query_units=units, use_bias=use_bias,
                                          key_units=units, value_units=units, num_heads=num_heads)
        elif attention_cell == 'my_multi_head_2':
            base_cell = DotProductAttentionCell(scaled=scaled, dropout=dropout)
            return MyMultiHeadAttentionCell2(base_cell=base_cell, query_units=units, use_bias=use_bias,
                                          key_units=units, value_units=units, num_heads=num_heads)
        else:
            raise NotImplementedError
    else:
