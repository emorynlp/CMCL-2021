...
__all__ = ['AttentionCell', 'MultiHeadAttentionCell', 'MLPAttentionCell', 'DotProductAttentionCell',
            'MyMultiHeadAttentionCell', 'MyMultiHeadAttentionCell2']
...
class MultiHeadAttentionCell(AttentionCell):
    ...
    def _read_by_weight(self, F, att_weights, value):
        ...
        # Shape: (batch_size, query_length, num_heads * ele_units)
            # reshape 1: (batch_size, num_heads, query_length, ele_units)
            # transpose: (batch_size, query_length, num_heads, ele_units)
            # reshape 2: (batch_size, query_length, num_heads * ele_units)
...
class MyMultiHeadAttentionCell(AttentionCell):
    def __init__(self, base_cell, query_units, key_units, value_units, num_heads, use_bias=True,
                 weight_initializer=None, bias_initializer='zeros', prefix=None, params=None):
        super(MyMultiHeadAttentionCell, self).__init__(prefix=prefix, params=params)
        self._base_cell = base_cell
        self._num_heads = num_heads
        self._use_bias = use_bias
        ...
        for name, unit in units.items():
            ...
            with self.name_scope():
                unit = int(unit/self._num_heads)
                for i in range(num_heads):
                    setattr(
                        self, 'proj_{}{}'.format(name, i),
                        nn.Dense(units=unit, use_bias=self._use_bias, flatten=False,
                                 weight_initializer=weight_initializer,
                                 bias_initializer=bias_initializer, prefix='{}{}_'.format(name, i)))

        name = 'attn_output'
        with self.name_scope():
            unit = int(query_units/self._num_heads)
            for i in range(num_heads):
                setattr(
                    self, 'proj_{}{}'.format(name, i),
                    nn.Dense(units=unit, use_bias=self._use_bias, flatten=True,
                             weight_initializer=weight_initializer,
                             bias_initializer=bias_initializer, prefix='{}{}_'.format(name, i)))


    def __call__(self, query, key, value=None, mask=None):
        return super(MyMultiHeadAttentionCell, self).__call__(query, key, value, mask)

    def _get_layer(self, name, idx):
        return getattr(self, 'proj_{}{}'.format(name, idx))

    def hybrid_forward(self, F, query, key, value=None, mask=None):  # pylint: disable=arguments-differ
        multi_heads_outputs = None
        multi_heads_att_weights = None
        for i in range(self._num_heads):
            query = self._get_layer('query', i)(query)
            key = self._get_layer('key', i)(key)
            value = self._get_layer('value', i)(value)

            # Shape: (batch_size, query_length, query_length)
            att_weights = self._base_cell._compute_weight(F, query, key, mask)

            # Shape: (batch_size, query_length, ele_units)
            output = self._base_cell._read_by_weight(F, att_weights, value)

            # Shape: (batch_size, 1, ele_units)
            proj_layer = self._get_layer('attn_output', i)
            output = proj_layer(output)

            if i == 0:
                multi_heads_outputs = output
                multi_heads_att_weights = att_weights
            else:
                multi_heads_outputs = mx.symbol.Concat(multi_heads_outputs, output, dim=1)
                multi_heads_att_weights = mx.symbol.Concat(multi_heads_att_weights, att_weights, dim=1)

        return multi_heads_outputs, multi_heads_att_weights


# use pooling
class MyMultiHeadAttentionCell2(AttentionCell):
    def __init__(self, base_cell, query_units, key_units, value_units, num_heads, use_bias=True,
                 weight_initializer=None, bias_initializer='zeros', prefix=None, params=None):
        super(MyMultiHeadAttentionCell2, self).__init__(prefix=prefix, params=params)
        self._base_cell = base_cell
        self._num_heads = num_heads
        self._use_bias = use_bias
        self._units = query_units
        ...
        self._sub_units = int(query_units/self._num_heads)
        self.pooler = self._get_pooler(self._sub_units, 'pooler_')


    def __call__(self, query, key, value=None, mask=None):
        return super(MyMultiHeadAttentionCell2, self).__call__(query, key, value, mask)
    ...
    def _apply_pooling(self, sequence):
        ...
        outputs = outputs.reshape(shape=(-1, self._sub_units))
